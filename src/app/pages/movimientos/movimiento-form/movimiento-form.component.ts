...
export interface EntradaSalidaGrua {
  ...
  grua_id: number;
  empleado_id: number;
  // Ya no necesitamos cliente_id en el frontend ni en el backend para este caso
  ...
  // Ya no necesitamos la relación con Cliente
...
// Asegúrate que esta interfaz coincide con lo que devuelve tu backend para Empleado
...
// Ya no necesitamos la interfaz Cliente
...
export class EntradaSalidaGruaService {
   ...
   * @param movimiento Los datos del registro de movimiento a crear (sin cliente_id).
  ...
  createMovimiento(movimiento: EntradaSalidaGrua): Observable<EntradaSalidaGrua> {
    // Asegurarse de no enviar cliente_id si por alguna razón estuviera en el objeto
    const dataToSend: any = { ...movimiento };
    delete dataToSend.cliente_id;
    delete dataToSend.cliente; // Eliminar la relación si estuviera presente
    return this.http.post<EntradaSalidaGrua>(this.apiUrl, dataToSend);
  }

  /**
   * Actualiza un registro de movimiento existente.
   * @param id El ID del registro de movimiento a actualizar.
   * @param movimiento Los datos actualizados del registro de movimiento (sin cliente_id).
  ...
  updateMovimiento(id: number, movimiento: EntradaSalidaGrua): Observable<EntradaSalidaGrua> {
    ...
    delete dataToSend.cliente_id;
    delete dataToSend.cliente; // Eliminar la relación si estuviera presente
    return this.http.put<EntradaSalidaGrua>(url, dataToSend); // Usamos PUT o PATCH según cómo lo definiste en Laravel (apiResource usa PUT/PATCH)
  }
  ...
  deleteMovimiento(id: number): Observable<void> {
    ...
    * @param startDate La fecha de inicio del rango (formato<\ctrl95>-MM-DD).
    * @param endDate La fecha de fin del rango (formato<\ctrl95>-MM-DD).
